...
class EntityAlien(object):
    ...
    def __init__(self, x, y, width, height, row, col, speed, canvas, parent, canShoot, UID, life = 30, SkinID = 1, EXP = 10):
        self.posx = x
        self.posy = y
        self.dx = 0
        self.dy = 0
...
#         self.form = canvas.create_rectangle(self.posx - self.width/2, self.posy - self.height/2, self.posx + self.width/2, self.posy + self.height/2, fill= 'white')
        image = PIL.Image.open("resources/img/SpaceInvader_Enemy" + str(SkinID) + ".png")
        image = image.resize((width, height))
        image.save("resources/img/imgResized/SpaceInvader_Enemy" + str(SkinID) + ".png", "png")
        self.pic = PhotoImage(file = "resources/img/imgResized/SpaceInvader_Enemy" + str(SkinID) + ".png")
